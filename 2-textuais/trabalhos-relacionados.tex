...
Neste capítulo serão descritas as fases realizadas durante a Revisão da Literatura do
trabalho.
...
da pesquisa foi organizada em três fases: Planejamento, Condução e Resultados que serão 
descritas nas próximas seções. 

\section{Planejamento}
Segundo \citeonline{KITCHENHAM2009}, o Planejamento é a fase de definição de cada passo 
...
assim, nas próximas seções, serão descritas as fases deste protocolo.

\subsection{Definição de Questões de Pesquisa (\gls{QP})}

\textbf{\gls{QP}1} - Qual a estratégia adotada no trabalho? 
    \begin{alineascomponto}
        \item Criação de um \textit{framework}
        \item Simulação
...
Após a definição das questões de pesquisas, foi definida a seguinte \textit{string} de busca: \textit{"cidades inteligentes" AND "framework" AND "problema do caixeiro viajante" }


\subsection{Definições de bases de dados}
De posse da \textit{string} de busca, foram selecionadas as bases de dados, conforme mostra o Quadro \ref{Bases}:
	\begin{quadro}[h!]	
		\centering
...
\textbf{Critérios de exclusão: }
    \begin{itemize}
        ...
        \item Estudos não disponíveis para \textit{download}.
    \end{itemize}

\section{Condução}
Na fase de condução, foi colocado em prática o planejamento definido na seção de Planejamento, dessa forma, a \textit{string} de busca 
foi inserida em cada base de dados, respeitando as especificações de cada base. De posse dos resultados da busca, foi realizado 
o \textit{download} dos estudos disponíveis. Em seguida, os trabalhos passaram pelos critérios de inclusão e exclusão. 
Na Tabela \ref{tab:quantidade-artigos}, é possível identificar a quantidade de trabalhos em cada base durante a busca, 
a realização do \textit{download} e a aplicação dos critérios de inclusão e exclusão.

\begin{table}[h!]
\centering
\Caption{\label{tab:quantidade-artigos} Quantidade de artigos em cada base de dados.}
\begin{tabular}{@{}lrrr@{}}
\toprule
\multicolumn{1}{c}{\textbf{Base de dados}} & \multicolumn{1}{c}{\textbf{Resultados da Busca}} & \multicolumn{1}{c}{\textbf{Downloads}} & \multicolumn{1}{c}{\textbf{Inclusão/Exclusão}} \\ \midrule \midrule
Google acadêmico                           & 26                                               & 13                                     & 3                                              \\
Periódicos da capes                        & 10                                               & 6                                      & 4                                              \\
SBC-OpenLib                                & 3                                                & 3                                      & 1                                              \\ \bottomrule
\end{tabular}
...
o trabalho de \citeonline{SILVA2024}, no qual os autores desenvolveram uma solução baseada em \gls{API}s externas para calcular 
...
A estratégia mais prevalente identificada na \gls{QP}1 foi o desenvolvimento e/ou uso de algoritmos de otimização, 
totalizando quatro trabalhos. \citeonline{PAZ2023} desenvolveu um algoritmo para otimizar o transporte urbano 
considerando o menor trajeto no menor tempo possível, evitando gargalos como congestionamentos. 
\citeonline{SILVA2022} utilizaram ferramentas externas de código aberto para resolver o Problema 
do Caixeiro Viajante em centros urbanos, aplicado ao contexto de entregas dos correios. \citeonline{VILLELA2017} 
propuseram um algoritmo para resolver uma variação do Problema do Caixeiro Viajante, denominada Caixeiro Alugador, 
visando otimizar viagens com aluguel de veículos entre cidades. Por fim, \citeonline{CHAVES2007} apresentaram um 
método heurístico híbrido para resolver aproximadamente o Problema do Caixeiro Viajante com Coleta de Prêmios.

	\begin{figure}[h!]
        \captionsetup{width=16cm}
		\Caption{\label{fig:qp1} Resultados da \gls{QP}1.}
		%\centering
		\UFCfig{}{
...
Ainda em relação à \gls{QP}1, na alternativa de Abordagem com Internet das Coisas, 
\citeonline{BARTH2016} desenvolveu uma metodologia para elaboração de um sistema computacional baseado 
...
de simulação, \citeonline{SATI2020} utilizaram e compararam três diferentes modelos matemáticos, analisando 
os benefícios de cada um para a resolução do problema de roteamento de veículos em centros urbanos
voltado ao transporte de funcionários. Quanto à alternativa de análise de dados, \citeonline{GEORGES2014} 
...
adotados nos trabalhos selecionados. \citeonline{PAZ2023} utilizou o Algoritmo Genético combinado com conceitos 
de probabilidade para otimização de rotas e, para fins comparativos, também empregou os algoritmos Colônia 
de Formigas, \gls{GRASP}, A* (A estrela) e Dijkstra. \citeonline{BARTH2016} adotou o algoritmo Colônia de Formigas em 
seu projeto de redes híbridas para cidades inteligentes. \citeonline{VILLELA2017} e \citeonline{CHAVES2007} utilizaram,
 ...
 Por sua vez, \citeonline{SATI2020} adotaram Programação Linear para resolver o problema de roteamento de veículos em centros urbanos.

    \begin{figure}[h!]
        \captionsetup{width=16cm}
		\Caption{\label{fig:qp2} Resultados da \gls{QP}2.}
		%\centering
		\UFCfig{}{
...
Os demais trabalhos, \citeonline{SILVA2024}, \citeonline{SILVA2022} e \citeonline{GEORGES2014}, não adotaram nenhum dos 
algoritmos especificados na \gls{QP}2. Estes estudos utilizaram ferramentas externas para resolução dos problemas 
...
práticas sem a necessidade de reimplentação dos algoritmos.
