...
sistemas de informação permitem que gestores públicos tomem decisões baseadas em dados, 
realizem o monitoramento de serviços em tempo real e promovam a interação direta com a 
população \cite{WEISS2015}. Ferramentas como dashboards interativos e plataformas de dados 
...
Sistemas Inteligentes de Transporte (ITS, do inglês \textit{Intelligent Transportation Systems}) 
...
metodologias como \textit{City Information Modeling} (CIM) permite a integração de dados 
...
Aplicações mobile para relato de problemas urbanos por parte da população, integradas a sistemas 
...
multi-agentes e Internet das Coisas (IoT) permite testar cenários de desenvolvimento urbano 
...
realizadas. A integração de tecnologias de geolocalização, sensores IoT e plataformas de 
...
Problemas de otimização são fundamentais na ciência da computação e em diversas aplicações 
práticas, como o planejamento de rotas, alocação de recursos e scheduling. Esses problemas 
buscam encontrar a melhor solução dentre um conjunto de soluções possíveis, de acordo com 
...
\begin{itemize}
    ...
    com grande espaço de busca, devido ao crescimento exponencial do número de soluções.
...
A principal limitação dos métodos exatos é a complexidade computacional. Muitos problemas 
...
\begin{itemize}
    ...
    \item \textbf{Especificidade:} Muitas heurísticas são desenvolvidas explorando características 
...
Os Algoritmos Genéticos (AG) constituem uma meta-heurística inspirada nos princípios da 
...
\begin{itemize}
    ...
    cromossomos pais para gerar descendentes. Para problemas de permutação como o TSP, 
    utiliza-se técnicas como o Cruzamento de Ordem (OX), que preserva a ordem relativa dos 
...
Os algoritmos de busca em grafos são fundamentais na ciência da computação e constituem 
a base para resolver diversos problemas de otimização, incluindo problemas de caminho mínimo 
e roteamento \cite{CORMEN2012}. Um grafo $G = (V, E)$ é composto por um conjunto de 
vértices $V$ e um conjunto de arestas $E$, onde cada aresta conecta dois vértices. Em grafos 
ponderados, cada aresta possui um peso associado, representando distância, custo ou tempo.

\subsection{Busca em Largura (BFS)}

A Busca em Largura (\textit{Breadth-First Search} - BFS) explora o grafo sistematicamente 
visitando todos os vértices a uma mesma distância da origem antes de avançar para níveis mais 
profundos \cite{CORMEN2012}. O algoritmo utiliza uma estrutura de fila (FIFO - \textit{First In, 
First Out}) para controlar a ordem de visitação dos vértices.
...
A Busca em Profundidade (\textit{Depth-First Search} - DFS) percorre o grafo explorando o 
máximo possível cada caminho antes de retroceder \cite{CORMEN2012}. Pode ser implementada 
recursivamente ou utilizando uma pilha (LIFO - \textit{Last In, First Out}). A DFS também possui 
complexidade $O(V + E)$.
...
\begin{itemize}
    ...
    \item Ordenação topológica em grafos acíclicos direcionados (DAG);
...
A Busca de Custo Uniforme (\textit{Uniform-Cost Search} - UCS) é uma estratégia de busca 
que expande o nó com menor custo de caminho acumulado, sendo uma generalização da BFS para 
...
custo de aresta. Na prática, com implementação eficiente usando heap binário, a complexidade 
é $O((V + E) \log V)$.
