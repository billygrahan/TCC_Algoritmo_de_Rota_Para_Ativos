...
Sistemas Inteligentes de Transporte (\gls{ITS}, do inglês \textit{Intelligent Transportation Systems}) 
...
metodologias como \textit{City Information Modeling} (\gls{CIM}) permite a integração de dados 
...
práticas, como o planejamento de rotas, alocação de recursos e \textit{scheduling}. Esses problemas 
...
A Busca em Largura (\textit{Breadth-First Search} - \gls{BFS}) explora o grafo sistematicamente 
visitando todos os vértices a uma mesma distância da origem antes de avançar para níveis mais 
profundos \cite{CORMEN2012}. O algoritmo utiliza uma estrutura de fila (\gls{FIFO} - \textit{First In, 
...
A Busca em Profundidade (\textit{Depth-First Search} - \gls{DFS}) percorre o grafo explorando o 
máximo possível cada caminho antes de retroceder \cite{CORMEN2012}. Pode ser implementada 
recursivamente ou utilizando uma pilha (\gls{LIFO} - \textit{Last In, First Out}). A \gls{DFS} também possui 
complexidade $O(V + E)$.
...
A Busca de Custo Uniforme (\textit{Uniform-Cost Search} - \gls{UCS}) é uma estratégia de busca 
