...
Sistemas Inteligentes de Transporte (do inglês, \gls{ITS}) 
...
metodologias como (do inglês, \gls{CIM}) permite a integração de dados 
...
amplamente empregado na ciência da computação para representar redes, mapas e sistemas diversos 
\cite{CORMEN2012}. Formalmente, um grafo $G$ é definido pelo par ordenado $G = (V, E)$, onde 
$V$ é um conjunto finito de vértices (ou nós) que representam entidades, e $E$ é um conjunto 
de arestas que representam as conexões entre os vértices, denotadas como pares $(u, v)$, 
onde $u, v \in V$.

Os grafos podem ser classificados quanto à direcionalidade das arestas. Em grafos 
\textbf{não-direcionados}, as arestas estabelecem conexões bidirecionais, onde $(u, v)$ 
é equivalente a $(v, u)$, representando relações simétricas como estradas de mão dupla. 
Já nos grafos \textbf{direcionados} (ou dígrafos), as arestas possuem orientação, 
indicando uma conexão de $u$ para $v$ sem necessariamente existir de $v$ para $u$, 
úteis para modelar ruas de mão única ou dependências entre tarefas.

Quanto à ponderação, grafos podem ter arestas com pesos ou custos associados, definidos 
por uma função $w: E \rightarrow \mathbb{R}$ que atribui valores numéricos representando 
distâncias, tempos, custos ou capacidades \cite{CORMEN2012}. Conceitos fundamentais incluem 
\textbf{adjacência} (vértices conectados por aresta), \textbf{grau} (número de arestas 
incidentes a um vértice), \textbf{caminho} (sequência de vértices conectados) e 
\textbf{ciclo} (caminho que retorna ao vértice inicial).

Computacionalmente, grafos podem ser representados de duas formas principais: 
(i) \textbf{Matriz de Adjacência} -- matriz $A$ de dimensão $|V| \times |V|$ onde 
$A[i][j]$ indica a existência e peso de arestas, ocupando espaço $O(V^2)$; 
(ii) \textbf{Lista de Adjacência} -- para cada vértice mantém-se uma lista dos adjacentes, 
mais eficiente para grafos esparsos com complexidade espacial $O(V + E)$ \cite{CORMEN2012}.

A Figura \ref{fig:grafo-exemple} ilustra um exemplo de grafo e sua representação por lista e matriz de adjacências.

\begin{figure}[h!]
    \centering
    \captionsetup{width=14cm}
    \Caption{\label{fig:grafo-exemple} Representação de um grafo não dirigido. 
    (a) Um grafo não dirigido G com cinco vértices e sete arestas. 
    (b) Uma representação de G por lista de adjacências. 
    (c) A representação de G por matriz de adjacências.}
    \UFCfig{}{
        \includegraphics[width=14cm]{figuras/Grafo_exemple.png}
    }{
...
A Busca em Largura (do inglês, \gls{BFS}) explora o grafo sistematicamente 
visitando todos os vértices a uma mesma distância da origem antes de avançar para níveis mais 
profundos \cite{CORMEN2012}. O algoritmo utiliza uma estrutura de fila (do inglês, \gls{FIFO})
, onde o primeiro que entrar é o primeiro que sai, para controlar a ordem de visitação dos vértices.
...
A Busca em Profundidade (do inglês, \gls{DFS}) percorre o grafo explorando o 
máximo possível cada caminho antes de retroceder \cite{CORMEN2012}. Pode ser implementada 
recursivamente ou utilizando uma pilha (do inglês, \gls{LIFO}), onde o último que entrar será o primeiro a sair. A \gls{DFS} também possui 
complexidade $O(V + E)$.
...
\begin{itemize}
    ...
    \item Ordenação topológica em grafos acíclicos direcionados (do inglês, \gls{DAG});
...
A Busca de Custo Uniforme (do inglês, \gls{UCS}) é uma estratégia de busca 
...
O algoritmo \gls{AE} é uma extensão da busca de custo uniforme que incorpora uma função heurística 
...
Para que o \gls{AE} garanta encontrar o caminho ótimo, a heurística deve ser admissível, ou seja, 
...
O \gls{AE} é particularmente eficiente quando existe uma boa heurística disponível, pois reduz 
...
A Figura \ref{fig:aestrela-exemplo} apresenta um exemplo de aplicação do algoritmo A* 
para encontrar o caminho mais curto em um grafo.

\begin{figure}[h!]
    \centering
    \captionsetup{width=14cm}
    \Caption{\label{fig:aestrela-exemplo} Exemplo de aplicação do algoritmo A* em um grafo com arestas ponderadas.}
    \UFCfig{}{
        \includegraphics[width=14cm]{figuras/exemple_AEstrela.png}
    }{
        \Fonte{elaborada pelo autor (2025).}
    }
\end{figure}
